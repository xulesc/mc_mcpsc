...
## CONFIGURATION
PROGRAMS = ['ce', 'tmalign', 'fast', 'maxcluster', 'gralign', 'usm'] #, 'urms', 'maxcmo']
PROGDIR  = 'programs'
DATADIR  = 'data'
WORKDIR  = 'work'
PDBEXTN  = '.ent'
...
class GRALIGN_PRE_PROCESSOR:
  ...
  def pre_process_all_to_all(self, pdbDir):
    for file in os.listdir(pdbDir):
      ...
      f1 = file.replace(PDBEXTN,'.gw'); f2 = file.replace(PDBEXTN,'.ndump')
      cmd = './%s -i %s/%s -c A -o %s/%s -d 12.0' %(self._binPath1, pdbDir, file, self._tmpDir, f1)
      proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
      ## print proc.stdout.readlines()
      cmd = './%s -i %s/%s -o %s/%s' %(self._binPath2, self._tmpDir, f1, self._tmpDir, f2)
      proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
      ## print proc.stdout.readlines()
    l1 = len(os.listdir(pdbDir))
    l2 = len(filter(lambda x : x.endswith(".gw"), os.listdir(self._tmpDir)))
    l3 = len(filter(lambda x : x.endswith(".ndump"), os.listdir(self._tmpDir)))
    print 'pdb file count: %d, contact map count: %d, signature file count: %d' %(l1,l2,l3)
    ...
    for line in proc.stdout.readlines():
      ...
      if line.find('Size=') != -1:
        if new_pair:
          ...
          res.append([f1, f2])
        data = line.split(' ')
        dom_chain = data[2]; chain = dom_chain.split(':')[1]
        dlen = data[3].replace('(','').replace(')','').replace('=','').replace('Size','')
        res[len(res) - 1] += [chain, dlen]

      if line.find('Alignment length') != -1:
        new_pair = True
        data = line.split(' ')
        res[len(res)-1] += [data[3], data[6].replace('A',''), data[9], data[12], data[19]]
    if len(res) > 1:
      print 'pair: %s-%s has multi chain domain' %(fname1,fname2)
    ## each entry in res: fname1, fname2, chain1, len1, chain2, len2, Alignment length, Rmsd, Z-Score, Gaps, Sequence identities    
    if len(res) == 0:
      res.append([f1,f2])
    with lock:
      CEOUT.write('%s\n' %' '.join(res[0])) 
  ...
  def process_pair(self, fname1, fname2):
    ...
    for line in proc.stdout.readlines():
      ...
      if line.find('Length of Chain_') != -1:
        ret[0].append(line.split(':')[1].replace('residues','').strip())
      if line.find('Aligned length') != -1:
        data = re.split('\s+', line)
        ret[0] += [data[2], data[4], data[6]]
      if line.find('TM-score=') != -1:
        ret[0].append(line.split(' ')[1])
    ## each entry in res: fname1, fname2, len1, len2, aligned length, RMSD, sequence id, tm1, tm2
    if len(ret) == 0:
      ret.append([f1,f2])
    with lock:
      TMOUT.write('%s\n' %' '.join(ret[0])) 
...
class FAST_HANDLER:
  ...
  def process_pair(self, fname1, fname2):
    global lock
    #pdb1 = fname1.split('/')[-1].replace(PDBEXTN,''); pdb2 = fname2.split('/')[-1].replace(PDBEXTN,'')
    #cmd = "echo 'FAST\n' >> work/perf/%s_%s'" %(pdb1, pdb2)
    #proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    cmd = './%s %s %s' %(self._binPath, fname1, fname2) #, pdb1, pdb2)
    proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ret = [[fname1.split('/')[-1].replace(PDBEXTN,''), fname2.split('/')[-1].replace(PDBEXTN,'')]]
    for line in proc.stdout.readlines():
      if line.find('RMSD=') != -1:
        data = line.replace('\n','').split(' ')
        ret[0] += [data[0].split('=')[1], data[3].split('=')[1], data[4].split('=')[1], data[5].split('=')[1]]
    with lock:
      FASTOUT.write('%s\n' %' '.join(ret[0]))    
...
class MC_HANDLER:
  ...
  def process_pair(self, fname1, fname2):
    ...
    cmd = './%s %s %s' %(self._binPath, fname1, fname2)
    proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    l1= '-1'; l2 = '-1'; pairs = '-1'; rmsd = '-1'; maxsub = '-1'
    dlen = '-1'; grmsd = '-1'; tm = '-1'; sid = '-1'
    f1 = fname1.split('/')[-1].replace(PDBEXTN,'')
    f2 = fname2.split('/')[-1].replace(PDBEXTN,'')
    ...
    for line in proc.stdout.readlines():
      try:
        ...
        if line.find('Pairs=') != -1:
          data = line.split(' ')
          pairs = data[3].replace(',',''); rmsd = data[5].replace(',',''); 
          maxsub = data[6].split('=')[1]; dlen = data[8];
          grmsd = data[10]; tm = data[11].split('=')[1].replace(',',''); 
          sid = data[12].split('=')[1]
      except:
        pass
...
class GR_HANDLER:
  ...
  def process_alltoall(self, dirName):
    ...
    f = open(q,'w')
    for file in filter(lambda x : x.endswith(".ndump"), os.listdir(dirName)):
      f.write('%s\n' %file.replace('.ndump',''))
    f.close()
    # ./GR-Align -q skolnick.lst -r ./skolnick -o results.txt
    cmd = '%s -q %s -r %s -o %s' %(self._binPath, q, dirName, '%s/results.txt' %dirName)
    proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    for line in proc.stdout.readlines():
      print line
...
class USM_HANDLER:
  ...
  def process_pair(self, fname1, fname2):
    ...
    x = comp(cm1); y = comp(cm2); xy = comp(cm1 + cm2); yx = comp(cm2 + cm1)
...
## PROCESS
start = timer()
gralign_pre_processor = GRALIGN_PRE_PROCESSOR('programs/CMap', 'programs/DCount', 'work')
...
#    psc_pairs.append(('%s/%s' %(DATADIR,pdb_files[x]), '%s/%s' %(DATADIR,pdb_files[y])))
#for line in open('../workspace/data/tsv.3.2.0.VS.1.75.outpairs'):
for line in open('../workspace/data/ground_truth'):
  ldata = line.split('\t')
  psc_pairs.append(('%s/%s.ent' %(DATADIR,ldata[0]), '%s/%s.ent' %(DATADIR,ldata[1])))
#cm_files = filter(lambda x : x.endswith(".gw"), os.listdir('work/gralign'))
#cm_pairs = []
#for x in range(0, len(cm_files)):
#  for y in range(x, len(cm_files)):
#    cm_pairs.append(('work/gralign/%s' %cm_files[x], 'work/gralign/%s' %cm_files[y]))
...
#usm_seconds = doMulti('%s/usm_results.txt' %WORKDIR, usm_process_pair, cm_pairs)
#print 'usm processed in %d seconds' %usm_seconds
#fast_seconds = doMulti('%s/fast_results_1.txt' %WORKDIR, fast_process_pair, psc_pairs)
#print 'fast processed in %d seconds' %fast_seconds
#FASTOUT.close()
#mc_seconds = doMulti('%s/mc_results_1.txt' %WORKDIR, mc_process_pair, psc_pairs)
#print 'maxcluster processed in %d seconds' %mc_seconds
#MC.close()
#tm_seconds = doMulti('%s/tm_results_1.txt' %WORKDIR, tm_process_pair, psc_pairs)
#print 'tmalign processed in %d seconds' %tm_seconds
#TMOUT.close()
#ce_seconds = doMulti('%s/ce_results_1.txt' %WORKDIR, ce_process_pair, psc_pairs)
